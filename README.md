...
To enable Groq vision OCR, set `ENABLE_AI_FEATURES=true`, `AI_PROVIDER=groq`, `GROQ_API_KEY`, and `AI_MODEL=openai/gpt-oss-120b`. Gemini and OpenAI use the same pattern with `AI_PROVIDER=gemini` / `GEMINI_API_KEY` or `AI_PROVIDER=openai` / `OPENAI_API_KEY`.
